...
def convert_graph_query_and_path(sample: str, numNodes=50):
    """
    Input:
    ...
    for e in graph_part.split("|"):
        ...
        nodes.add(v)
        if u == start:
            start_edge_idx.append(len(edges) - 1)
    
    new_root = np.random.randint(0, numNodes, 1)[0]
    new_root = str(new_root)
    while new_root in nodes:
        new_root = np.random.randint(0, numNodes, 1)[0]
        new_root = str(new_root)


    if edges[start_edge_idx[0]][1] == path_nodes[1]:
        edges[start_edge_idx[1]] = (new_root, edges[start_edge_idx[1]][1])
    else:
        edges[start_edge_idx[0]] = (new_root, edges[start_edge_idx[0]][1])
    nodes.add(new_root)
...
if __name__ == "__main__":
    ...
    train_size = 12800 * 2
    train_data = [ ]
    with open("/home/nlp/hnn5071/datasets/next_token_data/datasets/graphs/deg_2_path_5_nodes_50_train_200000.txt", "r") as f:
    ...
    test_size = 10000
    test_data = []
    with open("/home/nlp/hnn5071/datasets/next_token_data/datasets/graphs/deg_2_path_5_nodes_50_test_20000.txt", "r") as f:
